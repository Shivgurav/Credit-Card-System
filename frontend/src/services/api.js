...
// ✅ Single gateway URL — all requests go through port 8080
...
// ── Auth (/auth/**) ───────────────────────────────────────────
export const loginUser       = (data)              => GATEWAY.post("/auth/login", data);
export const registerUser    = (data)              => GATEWAY.post("/auth/signup", data);
export const requestOtp      = (email)             => GATEWAY.post(`/auth/request-otp?email=${email}`);
export const verifyOtp       = (email, otp)        => GATEWAY.post(`/auth/verify-otp?email=${email}&otp=${otp}`);
export const resetPassword   = (email, newPassword)=> GATEWAY.post(`/auth/reset-password?email=${email}&newPassword=${encodeURIComponent(newPassword)}`);
export const updateAddress   = (email, address)    => GATEWAY.put(`/auth/update-address?email=${email}&address=${encodeURIComponent(address)}`);
export const getUserByEmail  = (email)             => GATEWAY.get(`/auth/user?email=${email}`);

// ── Merchants (/merchants/**) ─────────────────────────────────
export const registerMerchant = (data) => GATEWAY.post("/merchants", data);
export const getAllMerchants   = ()     => GATEWAY.get("/merchants/allmerchants");
export const getMerchantById  = (id)   => GATEWAY.get(`/merchants/${id}`);
export const validateMerchant = (id)   => GATEWAY.get(`/merchants/validate/${id}`);

// ── Cards (/cards/**) ─────────────────────────────────────────
export const applyCard              = (data)     => GATEWAY.post("/cards/apply", data);
export const getPendingApplications = ()         => GATEWAY.get("/cards/applications/pending");
export const getAllApplications      = ()         => GATEWAY.get("/cards/applications");
export const reviewApplication      = (id, data) => GATEWAY.put(`/cards/applications/${id}/review`, data);
export const getCard                = (num)      => GATEWAY.get(`/cards/${num}`);
export const updateCardStatus       = (data)     => GATEWAY.put("/cards/status", data);
export const payBill                = (cardNumber, amount)       => GATEWAY.put(`/cards/${cardNumber}/pay-bill`, { amount });
export const requestPinOtp          = (cardNumber)               => GATEWAY.post(`/cards/${cardNumber}/request-pin-otp`);
export const setPin                 = (cardNumber, otp, pin)     => GATEWAY.post(`/cards/${cardNumber}/set-pin?otp=${otp}&pin=${pin}`);
export const verifyPin              = (cardNumber, pin)          => GATEWAY.post(`/cards/${cardNumber}/verify-pin?pin=${pin}`);
export const cashWithdrawal         = (cardNumber, amount)       => GATEWAY.put(`/cards/${cardNumber}/cash-withdrawal`, { amount });

// ── Transactions (/transactions/**) ──────────────────────────
export const postTransaction       = (data)        => GATEWAY.post("/transactions", data);
export const getTransactionsByCard = (cardNumber)  => GATEWAY.get(`/transactions/${cardNumber}`);
